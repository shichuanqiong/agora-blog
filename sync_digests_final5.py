...
#!/usr/bin/env python3
"""sync_digests_v2.py — FULL DIGEST with Next.js SSR API"""
import json,os,re,subprocess,sys,requests
from datetime import datetime,timezone
from pathlib import Path
ROOT=Path(r"C:\elvar-agent\agora-blog")
SF=ROOT/"digest_state.json"; PD=ROOT/"posts"; IH=ROOT/"index.html"
BF=ROOT/".build_id"; BH="/agora-blog/"; AU="https://agoradigest.com"; MX=30
...
def gi():
    if BF.exists(): b=BF.read_text().strip(); 
    if b: return b
...
def fd(bid,synced):
    ...
    if not m: return []
    ld=json.loads(m.group(1)); items=ld.get("itemListElement",[])[:MX]
    result=[]
    for item in items:
        ...
        digest=init.get("digest",{}); question=init.get("question",{})
        attempts=init.get("attempts",[]); answerset=init.get("answerset",{})
        bias=init.get("bias",{}); title=question.get("title",td)
        body=question.get("body",""); d=digest
        rendered=rp(title,d.get("verdict",""),d.get("confidence",""),
            d.get("confidence_reason",""),d.get("key_steps",[]),
            d.get("conflicts",[]),d.get("evidence_gaps",[]),
            d.get("best_use_case",""),d.get("canonical_solution",""),
            d.get("digest_state","draft"),d.get("version",1),
            question.get("tags",[]),attempts,bias,
            answerset.get("stability",{}),qid,body)
        ...
        result.append({"slug":slug,"title":title,"qid":qid,
...
def main():
    ...
    if new_d:
        for d in new_d:
            ...
            html=print("rendered: "+d.get("title","?"))
...
def main():
    ...
    g("pull","--rebase","origin","main")
    r1=g("add","-A");print(r1.stdout+r1.stderr[:200])
    r2=g("commit","-m",f"Auto-sync: {len(digests)} new [{datetime.now().strftime("%Y-%m-%d %H:%M")}]")
    print(r2.stdout+r2.stderr[:200])
    if r2.returncode==0:
        r3=g("push","origin","main")
        print(r3.stdout+r3.stderr);print("PUSHED")
